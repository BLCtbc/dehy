...
class BasketMiddleware:
	...
	def get_basket(self, request):
		...
		if hasattr(request, 'user') and request.user.is_authenticated:
			...
			new_basket = Basket()
			new_basket.strategy = request.strategy
			...
			if basket_content:
				for key, val in basket_content.items():
					...
					if product:
						...
						new_basket.add_product(product, val)

			if basket.is_empty and not new_basket.is_empty:
				self.merge_baskets(basket, new_basket)
